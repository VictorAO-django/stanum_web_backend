import MT5Manager
from trading.models import MT5Order

def save_mt5_order(order_obj:MT5Manager.MTOrder):
    """
    Save or update an MT5 order into Django database.
    order_obj: IMTOrder object from MT5 Manager API
    """
    order, _ = MT5Order.objects.update_or_create(
...
class OrderSink:
    def OnOrderAdd(self, order:MT5Manager.MTOrder):
        order.Print()
        save_mt5_order(order)

    def OnOrderUpdate(self, order:MT5Manager.MTOrder):
        order.Print()
        save_mt5_order(order)

    def OnOrderDelete(self, order:MT5Manager.MTOrder):
        order.Print()
        MT5Order.objects.filter(order=order.Order).delete()

    def OnOrderClean(self, login):
        MT5Order.objects.filter(login=login).delete()
